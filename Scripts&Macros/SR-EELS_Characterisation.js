 ...
 */
 ...
 /*
  * DRAFT - INFO
  * This is a draft version of the script. EFTEMj will contain a more enhanced version. This script should only be used by Figure6.ijm, to create Figure6.png for my MC2015 poster.
  */
...
function main() {
	...
	settings.path = IJ.getDirectory("Select the folder that contains the images");
	if (settings.path[settings.path.lengt - 1] != "\\") settings.path += "\\";
	settings.path += "QSinK7=-15%\\forFigure6\\"
...
function runCharacterisation(results) {
	...
	for (i = 0; i < images.length; i++){
		...
		limit = new Array();
		for (j in result) {
			xValues.push(settings.binning * result[j].y);
			yValues.push(settings.binning * result[j].x);
			leftValues.push(settings.binning * result[j].left);
			rightValues.push(settings.binning * result[j].right);
			widthValues.push(settings.binning * (result[j].right - result[j].left));
			limit.push(result[j].limit);
		}

		if (settings.polynomialOrder == 2) {
			fitType = CurveFitter.POLY2;
		}
		result.leftFit = new CurveFitter(xValues, leftValues);
		result.leftFit.doFit(fitType);
		var leftValuesFit = new Array(xValues.length)
		for (j in xValues) {
			leftValuesFit[j] = result.leftFit.f(xValues[j]);
		}
		result.centreFit = new CurveFitter(xValues, yValues);
		result.centreFit.doFit(fitType);
		var yValuesFit = new Array(xValues.length)
		for (j in xValues) {
			yValuesFit[j] = result.centreFit.f(xValues[j]);
		}
		result.rightFit = new CurveFitter(xValues, rightValues);
		result.rightFit.doFit(fitType);
		var rightValuesFit = new Array(xValues.length)
		for (j in xValues) {
			rightValuesFit[j] = result.rightFit.f(xValues[j]);
		}
		result.widthFit = new CurveFitter(xValues, widthValues);
		result.widthFit.doFit(fitType);
		var widthValuesFit = new Array(xValues.length)
		for (j in xValues) {
			widthValuesFit[j] = result.widthFit.f(xValues[j]);
		}
		plot = new Plot("Figure6", "x-position [px]", "y-position [px]");
		plot.setLineWidth(2);
		plot.setFrameSize(1000, 400);
		plot.setFont(Font.PLAIN, 24)
		plot.setMaxIntervals(16);
		...
		plot.addPoints("", xValues, rightValues, Plot.CIRCLE);
		plot.setColor(java.awt.Color.RED);
		plot.addPoints("", xValues, widthValues, Plot.CIRCLE);
		plot.addPoints("", xValues, widthValuesFit, Plot.LINE);
		plot.setColor(java.awt.Color.BLACK);
		plot.addPoints("", xValues, yValuesFit, Plot.LINE);
		plot.setColor(java.awt.Color.BLUE);
		plot.addPoints("", xValues, leftValues, Plot.CIRCLE);
		plot.addPoints("", xValues, leftValuesFit, Plot.LINE);
		plot.addPoints("", xValues, rightValuesFit, Plot.LINE);
		plot.addLegend("Position of the spectrum\nBorders of the spectrum\nWidth of the spectrum");
		if (!settings.useThresholding) plot.add("CROSS", xValues, limit);
		plot.setLimits(0, settings.camHeight, 0, settings.camWidth);
		plot.setLineWidth(1);
		plot.makeHighResolution("Figure6_HiRes", 5.0, false, true);
		plot.show();
	}
}
